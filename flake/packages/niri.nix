{
  flake.packages'.niri-config =
    ...
    let
      ...
      niri-sticky-window' = ''"${getExe niri-sticky-window}"'';

      noctalia' = ''"${getExe noctalia}"'';

      noctalia-ipc =
        target: args:
        ''${noctalia'} "msg" "${target}"${foldl' (accumulator: arg: ''${accumulator} "${arg}"'') "" args}'';
    in
    writeText "niri-config.kdl" ''
      binds {
        ...
        Mod+D hotkey-overlay-title="Run an Application: ${getName fuzzel}" { spawn "${getExe fuzzel}"; }

        Mod+E hotkey-overlay-title="Open File Explorer: ${getName fileExplorer}" { spawn "${getExe fileExplorer}"; }

        Super+L hotkey-overlay-title="Lock the Screen" { spawn ${
          noctalia-ipc "session" [
            "lock"
          ]
        }; }
      ...
      ${foldl'
        ...
        XF86AudioRaiseVolume allow-when-locked=true { spawn ${noctalia-ipc "volume-up" [ ]}; }
        XF86AudioLowerVolume allow-when-locked=true { spawn ${noctalia-ipc "volume-down" [ ]}; }
        XF86AudioMute        allow-when-locked=true { spawn ${noctalia-ipc "volume-mute" [ ]}; }
        XF86AudioMicMute                            { spawn ${noctalia-ipc "mic-mute" [ ]}; }
        ...
        XF86MonBrightnessDown allow-when-locked=true { spawn ${noctalia-ipc "brightness-down" [ ]}; }
        XF86MonBrightnessUp   allow-when-locked=true { spawn ${noctalia-ipc "brightness-up" [ ]}; }
      ...
      spawn-at-startup ${niri-sticky-window'}
